...
import { 
  Shield, Users, UserCheck, Settings, Search, CheckCircle, XCircle, 
  Plus, Minus, Save, RefreshCw, Filter, Key, Lock
} from 'lucide-react';
...
const Permissions = () => {
  ...
  const [roles, setRoles] = useState(['admin', 'reseller', 'consumer', 'viewer']);
  ...
  useEffect(() => {
    if (activeTab === 'systemadmin') {
      if (systemAdminSearchQuery.length >= 2) {
        ...
        if (systemAdminSearchQuery.length === 0 && systemAdminUsers.length === 0) {
          fetchSystemAdminUsers();
        }
      }
    }
  ...
  }, [permissions, resourceFilter, actionFilter, searchQuery]);
  ...
  const fetchPermissions = async () => {
    try {
      ...
      if (result.error) {
        ...
        const permissionsList = Array.isArray(result) ? result : [];
  ...
  const fetchRolePermissions = async (role) => {
    try {
      ...
      if (result.error) {
        ...
        const permIds = result.map(rp => rp.permissions?.id || rp.permission_id).filter(Boolean);
  ...
  const fetchUserPermissions = async (userId) => {
    try {
      ...
      if (result.error) {
        ...
        const permIds = result.filter(up => up.granted !== false).map(up => up.permission_id || up.permission_name).filter(Boolean);
  ...
  const saveRolePermissions = async () => {
    try {
      ...
      const toAdd = selectedRolePermissions.filter(id => !currentPerms.includes(id));
      const toRemove = currentPerms.filter(id => !selectedRolePermissions.includes(id));
      ...
      setRolePermissions(prev => ({ ...prev, [selectedRole]: selectedRolePermissions }));
  ...
  const saveUserPermissions = async () => {
    ...
    try {
      setLoading(true);
      const result = await assignPermissionsToUser(selectedUser.user_id || selectedUser.id, userPermissions, true);
      if (result.error) {
        toast.error(result.error);
      } else {
  ...
  const handleSystemAdminToggle = async (userId, currentStatus) => {
    try {
      ...
      if (result.error) {
        ...
        toast.success(`System admin status ${newStatus ? 'granted' : 'revoked'}`);
  ...
  const filteredSystemAdminUsers = systemAdminUsers.filter(user => {
    if (!systemAdminSearchQuery || systemAdminSearchQuery.length < 2) return true;
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                borderBottom: activeTab === tab.id ? '3px solid #74317e' : '3px solid transparent',
        ...
        <div style={{ padding: '24px' }}>
          ...
          {activeTab === 'roles' && !loading && (
            <div>
              <div style={{
                ...
                <label style={{ fontWeight: '600', minWidth: '80px' }}>Select Role:</label>
                <select
                  value={selectedRole}
              ...
              <div style={{
                ...
                {permissions.map(permission => {
                  const isSelected = selectedRolePermissions.includes(permission.id);
                  return (
                    <div
                      key={permission.id}
                      ...
                      style={{
                        padding: '12px',
                        border: `2px solid ${isSelected ? '#74317e' : '#e0e0e0'}`,
                        borderRadius: '6px',
          ...
          {activeTab === 'users' && !loading && (
            <div style={{ display: 'flex', gap: '24px', height: '600px' }}>
              ...
              <div style={{
                ...
                <div style={{ flex: 1, overflowY: 'auto' }}>
                  {filteredUsers.map(user => {
                    ...
                    const isSelected = selectedUser && (selectedUser.user_id || selectedUser.id) === userId;
                    return (
                      <div
                        key={userId}
                        ...
                        style={{
                          ...
                          cursor: 'pointer',
                          backgroundColor: isSelected ? '#f3e5f5' : 'white',
                          borderLeft: isSelected ? '3px solid #74317e' : '3px solid transparent'
                        }}
                      >
              ...
              {/* User Info and Permissions Button */}
            ...
            <div>
              <div style={{
                ...
                <p style={{ margin: 0, fontSize: '14px', color: '#856404' }}>
                  System administrators have full access to all permissions and can manage the permission system.
                  Use this section to grant or revoke system administrator status.
                </p>
              </div>
              ...
              <div style={{ marginBottom: '24px', position: 'relative' }}>
                ...
                {systemAdminSearchQuery.length >= 2 && systemAdminSearchSuggestions.length > 0 && (
                  <div style={{
                    position: 'absolute',
                    top: '100%',
                    left: 0,
                    ...
                    {systemAdminSearchSuggestions.map(user => {
                      ...
                      return (
                        <div
                          key={userId}
                          onClick={() => {
                            setSystemAdminSearchQuery(user.email || user.full_name || '');
                            setSystemAdminSearchSuggestions([]);
                            fetchSystemAdminUsers();
                          }}
                          style={{
                ...
                {filteredSystemAdminUsers.map(user => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        padding: '16px',
                        border: `2px solid ${isSystemAdmin ? '#74317e' : '#e0e0e0'}`,
                        borderRadius: '6px',
                    ...
                    >
                      <div style={{
                        ...
                        <button
                          onClick={() => handleSystemAdminToggle(userId, isSystemAdmin)}
                          disabled={loading}
                          style={{
                            padding: '8px 16px',
                            backgroundColor: isSystemAdmin ? '#dc3545' : '#74317e',
                            color: 'white',
                            border: 'none',
                            borderRadius: '6px',
                            cursor: loading ? 'not-allowed' : 'pointer',
              ...
              {filteredSystemAdminUsers.length === 0 && (
                <div style={{ textAlign: 'center', padding: '40px', color: '#999' }}>
                  {systemAdminSearchQuery.length >= 2 ? 'No users found' : 'No users available'}
                </div>
              )}
            </div>
          )}
