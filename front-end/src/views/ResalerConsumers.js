...
// ✅ Using backend API for Resaler's Consumers
...
const ResalerConsumers = () => {
  console.log('👥 Resaler Consumers component rendering...');
  ...
  // Fetch Resaler's consumers from backend API
  useEffect(() => {
    console.log('🔄 Resaler Consumers component mounted - Fetching my consumers...');
    const fetchConsumers = async () => {
      try {
        setLoading(true);
        console.log('🔄 Calling getMyConsumers API...');
        const result = await apiClient.resalers.getMyConsumers();
  ...
  const handleAction = async (action, userId, userName) => {
    ...
    if (action === 'Update') {
      ...
      try {
        const result = await apiClient.resalers.resetMyConsumerPassword(userId);
  ...
  const handleCreateConsumer = async (consumerData) => {
    try {
      // Call backend API to create consumer (as resaler)
      const result = await apiClient.resalers.createMyConsumer(consumerData);
  ...
  const handleUpdateConsumer = async (updatedConsumer) => {
    try {
      // Call API to update consumer using user_id (as resaler)
      const result = await apiClient.resalers.updateMyConsumer(updatedConsumer.user_id, {
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      // Call backend API to delete consumer (as resaler)
      const result = await apiClient.resalers.deleteMyConsumer(deleteUserData.id);
...
export default ResalerConsumers;